...
- **공시·뉴스·시세 결합 분석**:
  - `news_gongsi_lab.py` — 공시/뉴스(`/news/gongsi/*`·`/news/news/*`)를 시세·수급·공매도·대차와 결합.
    이벤트 스터디(`event_study`), 전종목 기간 벌크 스크리닝(`market_period_screen` — KOSPI는 `hist_info_port` 날짜루프), 감성·뉴스량
  - `scenarios_price_first.py` — 시세→공시 스크리너(PS1~PS7: 등락률·거래대금·변동성·거래대금급증·신고가·종합주목도 → 각 종목 공시목록)
...
- [checkapi_bugreport_basic_info_all_port.txt](checkapi_bugreport_basic_info_all_port.txt):
  ...
  **당사 우회 해결**: codelist의 각 코드를 작은따옴표로 감싸면 영숫자 코드도 정상 조회됨(실측 검증).
  스킬 헬퍼(`_common.quote_codelist`)가 모든 `*_port` 호출에 자동 적용.
