...
왜 필요한가
  ...
  ※ '기세'가 아니다. 명세 확정은 nxt_krx_ingest.py 의 F30614 주석 참조.
    저장된 69 행의 ts/price/qty 는 '그 시점의 직전 실체결'의 복제다 -- 그래서 언제나 같은
    (일,종목)에 값이 같은 실체결 행이 따로 존재한다. 진짜 예상체결 값은 응답의 다른 칸
    (F30531/F15176/F15308)에 있는데 TICK_FIELDS 에 없어 저장되지 않을 뿐이다.
...
# 부분집합 크기 상한. None = 무제한(후보 수까지). 2026-08-16 에 6 에서 무제한으로 바꿨다.
#
# 6 은 '종목당 비69 평균 0.7행'을 근거로 잡은 값인데, 평균만 보고 꼬리를 안 봤다.
# 거래량 최상위 종목은 후보가 3,000행대라 비69 도 10행을 넘는다. 그러면 진짜 해가 탐색
# 범위 밖에 있고, 그 안에서 우연히 합이 맞는 가짜 해가 하나 있으면 그걸 유일해로 확정한다.
# 절단됐다는 사실 자체가 신호로 안 나와서 진짜 유일해와 구분되지 않는다 -- 조용한 오답이다.
#   실례 005930 2026-05-13: 진짜 비69 는 10행인데 6행짜리 가짜 해로 확정됐다.
#   후보 3,320행의 고유 가격이 2개(99%가 284,000원)라 금액 제약이 수량 제약과 겹쳐
#   사실상 제약이 하나로 줄었고, 고유 수량도 9가지뿐이라 같은 합이 흔했다.
#   MAXK 를 8 로만 올려도 해가 3개 나와 ambiguous 로 빠진다.
#
# 무제한으로 두는 비용이 거의 없다(2026-08-16 실측, 2,022건 전수):
#   MAXK  6 -> 확정 2,022 / 무제한 -> 확정 1,837 (ambiguous +185)
#   50 과 무제한의 차이가 1건뿐이고, 노드 예산 소진 0건, 42거래일 264초.
#   묶음 세기 덕에 후보가 수천 행이어도 고유 (수량,금액)이 몇 개뿐이라 탐색이 작다.
# 확정률은 98.4% -> 97.6% 로 떨어지지만, 떨어진 만큼이 '틀린 확정'이었다.
MAXK = None
...
def solve(rows, target_q, target_v, maxk=MAXK, cap=CAP):
    ...
    if maxk is None:
        maxk = len(rows)        # 무제한. 상한은 노드 예산이 대신 잡고, 걸리면 Unsearchable
