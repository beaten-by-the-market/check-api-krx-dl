...
def event_study(name, jcode, edate, want_short=False, want_loan=False, want_gongsi=False,
    ...
    if want_gongsi:
        ...
        try:
            ...
            out["공시타임라인"] = [{"일자": r["DATE"], "시간": r["TIME"][:4], "유형": classify_gongsi(r["TITLE"]),
                                 "제목": strip_corp(r["TITLE"])} for r in
                                sorted(gj, key=lambda r: r["DATE"] + r["TIME"])]
    ...
    if want_news:
        ...
        try:
            ...
            byd = collections.Counter(r["DATE"] for r in nj)
...
def disclosures_of(jcode, s, e, cats=None):
    ...
    out = []
    for r in sorted(gongsi_stock(jcode, s, e), key=lambda r: r["DATE"] + r["TIME"]):
        if "(정정)" in r["TITLE"]:
            continue
        c = classify_gongsi(r["TITLE"])
        if cats and c not in cats:
            continue
        out.append({"일자": r["DATE"], "시간": r["TIME"][:4], "유형": c,
                    "제목": strip_corp(r["TITLE"])[:50], "코드": r["CODE"], "MTVCD": r["MTVCD"]})
    return out
