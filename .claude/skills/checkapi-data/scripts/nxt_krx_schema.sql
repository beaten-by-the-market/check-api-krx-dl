...
CREATE TABLE IF NOT EXISTS nxt_tick (
  ...
  -- side: 공격 방향. 2026-09-05 실측(005930 06-08 정규장 52만 행, 체결가 위치와 교차)
  --   1 = 매수 공격(체결가 == ask1 이 98%)   4 = 매도 공격(체결가 == bid1 이 97%)
  --   2, 5 = 각각 1, 4 의 소수 변종(0.4%·0.4%). 0, 3 은 69 행에서만 보인다(API 3, 캡처 0).
  --   스프레드 안쪽 체결(레벨 소진 후 호가가 옮겨간 경우)의 방향은 이걸로만 알 수 있다.
  side       TINYINT      NULL,
  ...
  --
  -- ※ 이 호가는 그 체결이 반영된 '직후'의 호가다. 체결 하나 단위로 찍힌다(2026-09-05 실측).
  --   근거: (1) qty > ask_qty1 인 ask 체결이 0.7~10% 있다 -- 직전 호가면 불가능.
  --         (2) 연속 체결쌍에서 ask_qty1[i]-qty[i+1]==ask_qty1[i+1](직후가설) 28.4% vs
  --             ask_qty1[i]-qty[i]==ask_qty1[i+1](직전가설) 0.2%.
  --         (3) 같은 ts 묶음 안에서도 호가가 행마다 다르다(전부 동일 0.1%) -- 스윕 단위가 아니다.
  --   유효스프레드에 그대로 쓰면 안 된다. 레벨을 소진한 체결은 호가가 이미 옮겨가 체결가가
  --   스프레드 안쪽에 떨어지고(전체의 7~17%), 그 행들의 ES 가 0 근처로 붕괴한다. 종목 전체로는
  --   6~17% 과소(005930 0.92x, 035420 0.83x, 016360 0.85x). 직전 미드는 이렇게 만든다:
  --     매수 공격(side 1,2): 직전 ask = 체결가, bid = 이 행의 bid1  ->  mid = (price + bid1)/2
  --     매도 공격(side 4,5): 직전 bid = 체결가, ask = 이 행의 ask1  ->  mid = (ask1 + price)/2
  --   맞은 쪽 호가는 체결가 그 자체이고 반대쪽은 그 체결로 안 움직인다는 사실을 쓴다.
  --   직전 행의 호가를 쓰는 건 더 나쁘다 -- 체결 사이의 호가 변경(주문·취소)이 여기 없다
  --   (안쪽 체결 중 직전 행 터치와 일치 7.2%뿐).
  --
  --   숫자 예. 장부가 ask1 295,500 x 100주 / bid1 295,000 x 300주, 미드 295,250.
  --     누가 100주를 시장가로 산다 -> 295,500 에 100주 체결. 진짜 ES = 2*(295,500-295,250)/295,250 = 16.9bp.
  --     이 체결로 매도 1층이 소진돼 2층(296,000)이 올라온다. 행에는 '직후' 값이 찍힌다:
  --       price 295,500  qty 100  ask1 296,000  bid1 295,000     <- 체결가가 스프레드 '안쪽'
  --     같은 행 미드 = (296,000+295,000)/2 = 295,500 -> ES = 0bp. 250원 비싸게 산 체결이 공짜로 보인다.
  --     보정: side=1(매수)이니 직전 ask = 체결가 295,500, bid 는 이 행의 295,000 -> 미드 295,250 -> 16.9bp.
  --   터치를 정확히 친 체결(체결가 == ask1 또는 bid1)은 두 방식이 같은 값을 준다. 차이는 안쪽 체결에서만
  --   나고, 안쪽 체결은 체결가 위치로 방향을 못 정하므로 side 가 필수다.
  --   잔량 검정(위 근거 1·2)은 맞은 쪽 잔량만 본다: 체결가 == ask1 이면 ask_qty1, == bid1 이면 bid_qty1.
  --   매수 공격은 매도 쪽만 깎으므로 반대쪽 잔량은 그 체결과 무관하다.
  -- 체결가 위치(== ask1 / == bid1)로 공격 방향이 나오고, 위 보정을 거치면 유효 스프레드가 나온다.
