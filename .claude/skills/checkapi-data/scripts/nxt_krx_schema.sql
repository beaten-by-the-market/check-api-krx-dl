...
CREATE TABLE IF NOT EXISTS nxt_tick (
  ...
  -- 69 = 실체결 아님 -> 거래량·거래대금에서 제외한다.
  --
  -- 69 는 '기세'가 아니다(2026-08-13 전수 확인). 기세는 규정상 당일 무거래 종목에만 성립하고,
  -- 수량이 0이며, 종목·일자당 값 하나다. 실측은 셋 다 어긋난다 -- 69 보유 483종목이 전부
  -- 그날 실체결도 했고(2026-08-12), 수량이 있고 음수까지 나오며(2,633/11,222행, 최소 -10,000),
  -- 종목당 평균 23행이다. 명세 원문은 '69:예' 에서 잘려 있고 같은 코드계 F15317 은 '69:예상체결'.
  --
  -- 주의: 69 의 내용물이 수집 경로(src)마다 다르다. 값만 같고 다른 것이다.
  --   src=NULL(API)  메인마켓 마지막 체결을 15:20 마감 뒤에도 반복 재송신한 중복행.
  --                  체결시간이 마지막 체결 시각에 얼어붙는다(005380 06-08: 858행 전부
  --                  15:19:59·634,000·157 동일). 전 종목 중복률 95.6%. 수량에 정보 없음.
  --   src=1(캡처)    애프터 개장 단일가의 예상체결 수량 증분. 15:30:00~15:39:59 실시각,
  --                  하향 정정이 음수. 누적합 = 15:40:00 개장 체결량(원 단위 일치, 94~96%).
  --
