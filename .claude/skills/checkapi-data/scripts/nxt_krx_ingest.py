...
#
# 중요: 깃발은 양쪽 다 '예상체결'인데, 경로에 따라 '값'이 다르다.
#   src=1 (수집서버 1313 캡처 이관분, client_import/import_ticks.py)
#       진짜 예상체결 값이 들어 있다. 15:30:00~15:39:59 실시각, 수량은 증분이라 하향 정정이
#       음수로 온다. 누적합이 15:40:00 개장 단일가 체결량과 원 단위로 일치한다
#       (005930 2026-08-12: 69 합 35,082 = 개장 체결 256,000 x 35,082. 전 종목 94~96%).
#       side=0 에 실리고, 캡처분에서는 side=0 <-> 69 가 완전 동치다(3일 33,418행 예외 0).
#   src=NULL (CHECK API /tick_date, 이 스크립트)
#       예상체결의 가격·수량을 싣지 못하고 '그 시점의 직전 실체결'을 그대로 복사해 넣는다.
#       tick_date 가 체결 파일 재생이라 예상체결 값을 보관하지 않는 것으로 보인다.
#       그래서 69 행은 언제나 같은 (일,종목)에 있는 실체결 행의 완전한 복제다.
#       전수 확인(2026-08-15, 정답 28거래일 271,602행): 69 시각이 그 종목의 마지막 실체결보다
#       늦은 경우 0건. 같은 시각 36.1%, 이른 시각 63.9% -- '이른' 쪽은 그 시점의 직전 체결이다.
#       (일,종목)의 79.6%는 69 가 한 시각에만 몰린다(거래가 마감에 멈춘 종목).
#
#       예 (2026-05-04):
#         005930  69 2,496행이 19가지 값의 반복. 15:19:59 x46, 15:30:15 x65 ... 15:39:32 x99.
#                 부풀린 수량 73,734주 / 공식 22,786,734주 = 0.32%
#         000660  15:19:59 은 1,449,000원인데 15:30:26 부터 1,447,000원으로 바뀐다 --
#                 그 사이 실체결이 생기자 재통보 값이 따라 갱신됐다. 복제라는 직접 증거다.
#         002030  그날 375주만 거래된 종목. 69 는 딱 1행(15:19:59 273,000원 x 15주)
#         016800  거래가 더 적으면 69 가 아예 없기도 하다(그날 23행, 69 0)
