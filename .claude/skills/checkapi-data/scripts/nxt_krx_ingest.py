...
#
# 69 = 실체결 아님. 거래량에서 제외한다.
#   ※ 69 는 '기세'가 아니다(2026-08-13 전수 확인). 기세는 규정상 (1) 당일 체결이 하나도
#     없는 종목에만 성립하고 (2) 수량이 0이며 (3) 종목·일자당 값 하나다. 실측은 셋 다 어긋난다:
#     69 보유 483종목이 전부 그날 실체결도 했고(2026-08-12), 수량이 있으며 음수까지 나오고
#     (2,633/11,222행, 최소 -10,000), 종목당 평균 23행·최대 2,142행이다.
#     KOSCOM 명세 원문은 '69:예' 에서 잘려 있고(괄호도 안 닫힘) 같은 코드계 F15317 은
#     '69:예상체결' 이다. 아래 실측은 F15317 쪽 해석과 맞는다.
#
# 중요: 69 의 '내용물'이 수집 경로(nxt_tick.src)마다 다르다. 값은 같은 69 인데 다른 것이다.
#   src=NULL (CHECK API /tick_date, 이 스크립트)
#       메인마켓 마지막 체결을 15:20 마감 뒤에도 반복 재송신한 중복행. 체결시간이
#       마지막 체결 시각에 얼어붙는다(005380 2026-06-08: 858행 전부 15:19:59·634,000·157
#       동일, 같은 값의 진짜 체결이 side=4 로 따로 존재). 전 종목 중복률 95.6%
#       (16,297행 -> 고유 (ts,price,qty) 713개). 수량 자체에는 정보가 없다.
#       side=3 에 실린다. side=3 중 69 아닌 행은 2.6%.
#   src=1 (수집서버 1313 캡처 이관분, client_import/import_ticks.py)
#       애프터마켓 개장 단일가의 예상체결 수량 증분. 15:30:00~15:39:59 실시각에만 있고
#       하향 정정이 음수로 들어온다. 누적합이 15:40:00 개장 체결량과 원 단위로 일치한다
