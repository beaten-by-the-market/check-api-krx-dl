...
5. **아껴야 할 자원은 호출 수가 아니라 응답 바이트다.** 일 사용량 한도 **1,000,000,000 bytes(=1GB,
   `cust_id` 단위)** 가 있다. 대량 수집 전 **`data_list` 로 필요한 F-code만 지정**할 것(123필드→3필드에서
   1.1GB→48MB 실측). ⚠ `data_list` 는 **없는 F-code를 오류 없이 조용히 버린다** → 요청·반환 개수를 대조한다.
6. **`success:false` 를 빈 결과로 처리하지 말 것.** 한도 초과·rate limit은 HTTP 200 + `success:false` 로
   온다. `results or []` 로 흘리면 **"데이터 0건"으로 조용히 기록**된다(실제 사고: 코스닥 305일이 빈 값으로
   채워진 채 "완료" 출력). 예외로 올리고, 완료 시 **커버 범위를 출력해 완전성을 검사**한다.
7. **시계열 조회는 초당 1회 제한.** `hist_info` 등을 반복하면 `시계열 데이터의 조회는 1초에 1회로
   제한됩니다.` 가 뜬다 → 호출 간 **≥1.15초** 간격. (동시 호출 금지는 `references/usage.md` 참조.)
...
### 5.7) 장중 flash crash / 급등 탐지 (1분봉 미시구조)
히스토리컬 1분봉(`/stock/m00X/intra_date`, edate로 과거일)에서 **직전 봉 대비 순간 급락/급등 후 되돌림**을
잡고 거래량급증·지수동반·당일공시로 원인을 규명하는 건 `scripts/flash_lab.py`로 돌린다.
시나리오·구조·함정은 **`references/flash-intraday-analysis.md`** 참조(전부 실호출 검증됨).
```python
import flash_lab as fl    # scripts/ 에서 실행, 등록 IP·샌드박스 밖
...
### 5.8) NXT(넥스트레이드) 거래소별 분석
**거래소별 stock 패밀리(실측)**: m001/m003=KRX(코스피/코스닥) · **m222/m223=NXT** · **m224/m225=통합(KRX+NXT)** ·
...
```
핵심(상세는 레퍼런스): NXT 첫 거래일 **2025-03-24**(출범 3/4엔 각 시장 5종목뿐) · **NXT에 ETF/ETN 없음**(F34501에 EF/EN 부재) ·
**code_info는 현재 상장분만** → 상폐 포함 과거 유니버스는 **`rank_invest_date`(sdate/edate 시점 유니버스)**,
상폐 종목 과거 시세는 **hist_info에 그대로 보존** · **상장 ≠ 거래**(rank_invest_date 행 = 그날 상장분, 실거래는
매도 `F06505_12` 또는 매수 **`F06507_12`** > 0 — **F06506은 없는 코드**) · **과거 일별 전종목 시세 endpoint는 없다**
(rank는 당일만) · `rank_invest_date` 기간 **1년 초과 불가**, 넓은 범위는 **502** → 월 단위 분할.
실시간 호가 DB적재는 `scripts/hoga_ingest.py`+`references/hoga-ingest-setup.md`.
...
- `references/nxt-analysis.md`: NXT/통합 거래소 패밀리, 일별 실거래 유니버스, 상폐 종목 포함법, 일 사용량 한도·data_list·rate limit. 헬퍼 `scripts/nxt_universe.py`
- `references/investor-codes.md`: 투자자 구분 번호(1~20) → 이름 범례 (외국인/기관/개인 등)
- `references/usage.md`: 인증/IP제한/POST/F-code/한도·rate limit/결합 규칙 요약
