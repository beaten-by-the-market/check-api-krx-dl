...
m226~m229=NXT/통합 업종·투자자. NXT/통합도 hoga_info(46필드=10단계)·intra_date 등 동일 endpoint 보유 →
**NXT·통합 호가가 API에 존재**(과거 "NXT 없음" 서술 정정). ⚠ spec 메뉴 state=off·"NXT 활성화되면 반영" 표기 →
**live 반환은 장중 실측 필요**. 실시간 호가 DB적재는 `scripts/hoga_ingest.py`+`references/hoga-ingest-setup.md`.
