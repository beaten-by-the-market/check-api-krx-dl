...
### 실행 시 핵심 주의 — 샌드박스 밖에서
실데이터 스크립트(`call_checkapi`·`market_brief`·`kosdaq_daily`·`short_daily`·`cache_codes`·`news_gongsi_lab`·`scenarios_price_first`)는 **샌드박스 밖**에서 돌려야 한다.
...
| `call_checkapi`·`market_brief`·`kosdaq_daily`·`short_daily`·`news_gongsi_lab`·`scenarios_price_first`·`cache_codes`·`search_codes`(캐시 필요) | ❌ IP 거부/캐시 없음 |
...
## MCP 서버 (Claude Desktop / Claude Code에서 로컬 실행)

`mcp_server.py` — 검증된 스킬 로직을 감싼 **로컬 stdio MCP 서버**.
Claude Desktop이 **자동 실행**하므로 파워쉘로 미리 켜둘 필요 없다(config 등록 → 앱 재시작이면 끝).
로컬 프로세스라 CHECK 호출이 **이 PC(등록 IP)** 로 나가 실데이터가 된다.

**노출 툴**: `search_endpoints`(endpoint 검색) · `get_endpoint_spec`(파라미터·응답필드) ·
`call_checkapi`(실데이터, F-code 자동 디코딩) · `search_codes`(코드 카탈로그 검색).

### 설치
```bash
pip install mcp          # config의 command가 쓰는 python에 설치
# search_codes 쓰려면 캐시 1회(등록 IP): python .claude/skills/checkapi-data/scripts/cache_codes.py
```
...
```
→ 저장 후 **Claude Desktop 재시작**. 앱이 서버를 자동 spawn(수동 구동 X). 인증키는 이 파일(로컬)에만 있고
프로세스에 env로 전달 — 대화·클라우드로 안 간다. (`.env`가 리포에 있으면 env 생략 가능.)
※ `command`의 `python`이 안 잡히면 전체 경로(예: `C:/.../python.exe`)로. `args` 경로는 `/` 또는 `\`.

### Claude Code 등록
```bash
claude mcp add checkapi -- python C:/Users/Peter/github/check-api-krx-dl/mcp_server.py
# 인증키는 리포 .env 또는 --env 로 전달
```
→ 세션마다 Claude Code가 자동 실행.

### 요점
- **수동 상시가동 불필요** — 앱이 프로세스 생명주기 관리(켜짐/꺼짐).
- **IP**: 서버가 로컬 실행이라 등록 IP로 나감 → 실데이터 OK(터널·공개 불필요).
- 웹에서 쓰려면: 이 서버를 HTTP로 띄워 **터널(공개 URL)** 로 노출해야 함(상시가동+보안 필요) — 로컬 Desktop/Code면 불필요.
